...
// ==========================================
// IMPORT ROUTING FIX: MUST BE ABOVE `/:id` !
// ==========================================
...
router.get('/my/dses-stats', authenticate, async (req, res) => {
  try {
    ...
    // 1. Fetch real DSE user accounts registered to this dealer.
    const [realDses] = await db.query(`
      SELECT 
    ...
    // 2. Fetch virtual DSE stats from the leads table (names typed in but perhaps no user account yet).
    const [virtualDses] = await db.query(`
      SELECT 
        l.assigned_to_dse as full_name,
        COUNT(l.id) as total_assigned,
        COUNT(CASE WHEN l.status != 'Completed' THEN 1 END) as total_pending,
        COUNT(CASE WHEN l.status = 'Completed' THEN 1 END) as total_completed,
        COUNT(CASE WHEN l.follow_up_date IS NOT NULL AND l.status != 'Completed' THEN 1 END) as total_follow_up
      FROM leads l
      WHERE l.dealer_id = ? AND l.assigned_to_dse IS NOT NULL AND l.assigned_to_dse != ''
      GROUP BY l.assigned_to_dse
    `, [dealerId]);

    // 3. Merge them. Real accounts take precedence, virtual ones are appended as "Unregistered".
    const dseMap = {};
    realDses.forEach(d => { dseMap[d.full_name] = d; });

    virtualDses.forEach(vd => {
      if (!dseMap[vd.full_name]) {
        dseMap[vd.full_name] = {
          id: 'virtual_' + vd.full_name,
          full_name: vd.full_name,
          username: 'Unregistered',
          email: 'No Account Created',
          total_assigned: vd.total_assigned,
          total_pending: vd.total_pending,
          total_completed: vd.total_completed,
          total_follow_up: vd.total_follow_up
        };
      }
    });

    res.json({ success: true, data: Object.values(dseMap) });
  } catch (err) {
