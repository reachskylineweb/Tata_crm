      ...
      <div className="modal" style={{ maxWidth: 480 }}>
        ...
        <div className="modal-body">
          ...
          <div className="form-group">
            ...
            {lead.jio_tag_photo && !photo && (
              <div style={{ marginBottom: 12 }}>
                <div style={{ fontSize: '0.62rem', fontWeight: 800, color: 'var(--grey-400)', textTransform: 'uppercase', marginBottom: 6 }}>Current Photo</div>
                <img 
                  src={`${ASSET_BASE_URL}${lead.jio_tag_photo}`} 
                  alt="Existing Geo Tag" 
                  style={{ width: '100%', borderRadius: 10, border: '1px solid var(--grey-100)', maxHeight: 120, objectFit: 'cover' }} 
                />
              </div>
            )}
            <div
            ...
            >
              ...
              <div style={{ fontSize: '0.78rem', fontWeight: 600, color: photo ? 'var(--tata-blue)' : 'var(--grey-500)' }}>
                {photo ? photo.name : lead.jio_tag_photo ? 'Tap to Replace Photo' : 'Tap to Upload Geo Tag Photo'}
              </div>
            </div>
          ...
          {lead.jio_tag_photo && (
            <div style={{ marginTop: 20 }}>
              ...
              <a href={`${ASSET_BASE_URL}${lead.jio_tag_photo}`} target="_blank" rel="noreferrer">
                <img src={`${ASSET_BASE_URL}${lead.jio_tag_photo}`} alt="Geo Tag" style={{ width: '100%', borderRadius: 12, boxShadow: 'var(--shadow-sm)', border: '1px solid var(--grey-100)' }} onError={(e) => e.target.style.display='none'} />
              </a>
            </div>
          )}
        </div>
    ...
    <>
      ...
      <div className="card lead-table-desktop" style={{ border: 'none', boxShadow: 'var(--shadow-sm)', overflow: 'visible', borderRadius: 16 }}>
        <table className="data-table" style={{ width: '100%', tableLayout: 'fixed' }}>
          <thead>
            <tr>
              <th style={{ width: isDSE ? '16%' : '14%', paddingLeft: 12 }}>
                <MultiSelectFilter label="Name" options={nameOptions} selected={filters.name || []} onChange={(v) => onFilterChange('name', v)} />
              </th>
              <th style={{ width: isDSE ? '12%' : '10%' }}>
                <MultiSelectFilter label="Location" options={locationOptions} selected={filters.location || []} onChange={(v) => onFilterChange('location', v)} />
              </th>
              <th style={{ width: isDSE ? '14%' : '12%' }}>
                <MultiSelectFilter label="Phone" options={phoneOptions} selected={filters.phone || []} onChange={(v) => onFilterChange('phone', v)} />
              </th>
              <th style={{ width: isDSE ? '12%' : '10%' }}><MultiSelectFilter label="Vehicle" options={modelOptions} selected={filters.model || []} onChange={(v) => onFilterChange('model', v)} /></th>
              <th style={{ width: isDSE ? '18%' : '15%' }}><MultiSelectFilter label="Remark" options={REMARK_OPTIONS} selected={filters.remark || []} onChange={(v) => onFilterChange('remark', v)} /></th>
              {!isDSE && showFollowupDate && <th style={{ width: '13%' }}>Follow-up Date</th>}
              {!isDSE && <th style={{ width: '10%' }}><MultiSelectFilter label="DSE" options={dseOptions.length ? dseOptions : DSE_OPTIONS} selected={filters.allocation || []} onChange={(v) => onFilterChange('allocation', v)} /></th>}
              {showFieldInsight && <th style={{ textAlign: 'center', width: isDSE ? '8%' : '6%', fontSize: '0.68rem', fontWeight: 800, color: 'var(--grey-500)', textTransform: 'uppercase', letterSpacing: 0.5 }}>Log</th>}
              <th style={{ width: isDSE ? '12%' : '10%', paddingLeft: 10 }}><MultiSelectFilter label="Progress" options={statusOptions} selected={filters.progress || []} onChange={(v) => onFilterChange('progress', v)} /></th>
              {isDSE && <th style={{ width: '8%', paddingRight: 12 }}>Edit</th>}
            </tr>
          </thead>
          <tbody>
            {(leads || []).map((l) => (
              <tr key={l.id}>
                <td style={{ paddingLeft: 12 }}>
                  <div style={{ display: 'flex', flexDirection: 'column' }}>
                    <span style={{ fontWeight: 700, color: 'var(--tata-blue)', fontSize: '0.9rem', overflow: 'hidden', textOverflow: 'ellipsis', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }} title={l.full_name}>{l.full_name}</span>
                ...
                <td>
                  <div style={{ display: 'flex', flexDirection: 'column' }}>
                    <a href={`tel:${l.phone_number}`} style={{ display: 'inline-flex', alignItems: 'center', color: 'var(--green-600)', fontWeight: 700, fontSize: '0.8rem', textDecoration: 'none', whiteSpace: 'nowrap' }}><Phone size={10} style={{ marginRight: 4, flexShrink: 0 }} />{l.phone_number || '—'}</a>
                  </div>
                </td>
                <td>
                  <div style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '3px 6px', background: 'var(--tata-blue-50)', color: 'var(--tata-blue)', borderRadius: 6, fontSize: '0.7rem', fontWeight: 700, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: '100%' }}>
                  ...
                  <td>
                    <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                      <input 
                        type="date" 
                        className="form-control" 
                        style={{ height: 32, fontSize: '0.75rem', width: '100%', minWidth: 80, minHeight: 'auto', padding: '4px 8px' }} 
                        id={`date-${l.id}`}
...
export default function DealerLeads() {
  ...
  // A lead is scheduled if it has a follow-up date and that date is today or in the future
  const isScheduled = (l) => {
