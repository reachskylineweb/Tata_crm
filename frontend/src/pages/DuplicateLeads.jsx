  ...
  return (
    <div className="fade-in">
      ...
      <div className="card" style={{ border: 'none', boxShadow: 'var(--shadow-sm)', borderRadius: 16, overflow: 'hidden' }}>
        <div className="table-wrapper mobile-hide" style={{ border: 'none' }}>
              ...
              {loading ? (
                ...
                duplicates.map(lead => (
                  <tr key={lead.id}>
                    ...
                    <td>
                      <span style={{ fontSize: '0.75rem', fontWeight: 600, color: lead.reason?.includes('Invalid') ? 'var(--red-600)' : 'var(--orange-600)' }}>
        ...
        <div className="desktop-hide" style={{ padding: 12 }}>
          {loading ? (
             <div style={{ textAlign: 'center', padding: 40 }}><div className="spinner" /></div>
          ) : duplicates.length === 0 ? (
             <div style={{ textAlign: 'center', padding: 40 }}>No duplicates found</div>
          ) : (
            duplicates.map(lead => (
              <div key={lead.id} className="mobile-data-card">
                <div className="mobile-card-header">
                  <span style={{ fontWeight: 800, color: 'var(--tata-blue)', fontSize: '0.95rem' }}>{lead.full_name}</span>
                  <span style={{ fontSize: '0.65rem', color: 'var(--grey-400)', fontWeight: 600 }}>#{lead.id}</span>
                </div>
                <div className="mobile-card-row">
                   <span className="mobile-card-label">Lead Date</span>
                   <span className="mobile-card-value">{new Date(lead.lead_date).toLocaleDateString('en-IN')}</span>
                </div>
                <div className="mobile-card-row">
                   <span className="mobile-card-label">Phone</span>
                   <span className="mobile-card-value" style={{ color: 'var(--red-600)' }}>{lead.phone_number}</span>
                </div>
                <div className="mobile-card-row">
                   <span className="mobile-card-label">Location</span>
                   <span className="mobile-card-value">{lead.location}</span>
                </div>
                <div className="mobile-card-row" style={{ marginBottom: 0 }}>
                   <span className="mobile-card-label">Model</span>
                   <span className="mobile-card-value" style={{ color: 'var(--tata-blue)' }}>{lead.model || 'Unknown'}</span>
                </div>
              </div>
            ))
          )}
        ...
        {pagination.total_pages > 1 && (
          <div style={{ padding: '16px 24px', borderTop: '1px solid var(--grey-100)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--grey-50)' }}>
            <span style={{ fontSize: '0.8rem', color: 'var(--grey-500)', fontWeight: 600 }} className="mobile-hide">
              Showing {(pagination.page - 1) * pagination.limit + 1} to {Math.min(pagination.page * pagination.limit, pagination.total)} of {pagination.total}
            </span>
            <div style={{ display: 'flex', gap: 8, width: window.innerWidth < 768 ? '100%' : 'auto', justifyContent: 'space-between' }}>
              <button 
